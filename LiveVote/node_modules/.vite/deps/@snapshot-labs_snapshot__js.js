  ...
  send(envelop) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      return new Promise((resolve, reject) => {
        (0, import_cross_fetch.default)(address, init).then((res) => {
          var _a;
          if (res.ok)
...
function getSnapshots(network_1, snapshot_1, provider_1, networks_1) {
  return __awaiter(this, arguments, void 0, function* (network, snapshot, provider, networks2, options = {}) {
    const snapshots = {};
    networks2.forEach((n) => snapshots[n] = "latest");
    if (snapshot === "latest")
      return snapshots;
    const cacheKey = `${network}-${snapshot}-${networks2.join("-")}`;
...
function getHash(data, address) {
  const { domain: domain2, types, primaryType, message } = data;
  return typedData_exports.getMessageHash({ types, primaryType, domain: domain2, message }, address);
}
function verify(address_1, sig_1, data_1) {
  return __awaiter(this, arguments, void 0, function* (address, sig, data, network = "SN_MAIN", options = {}) {
    try {
      const contractAccount = new Contract2(ABI, address, getProvider$1(network, options));
      if (sig.length < 2) {
...
function getHash$1(data) {
  const { domain: domain2, types, message } = data;
...
function verify$1(address_1, sig_1, data_1) {
  return __awaiter(this, arguments, void 0, function* (address, sig, data, network = "1", options = {}) {
    const { domain: domain2, types, message } = data;
    try {
      const recoverAddress = verifyTypedData(domain2, types, message, sig);
    ...
    if (sig.endsWith(ERC6492_DETECTION_SUFFIX)) {
      try {
        return "0x01" === (yield provider.call({
          data: concat([
...
var SingleChoiceVoting = class _SingleChoiceVoting {
  constructor(proposal2, votes, strategies, selected) {
  ...
  getValidVotes() {
    return this.votes.filter((vote2) => _SingleChoiceVoting.isValidChoice(vote2.choice, this.proposal.choices));
  }
  getScores() {
    return this.proposal.choices.map((choice, i) => {
      const votes = this.getValidVotes().filter((vote2) => vote2.choice === i + 1);
...
var ApprovalVoting = class _ApprovalVoting {
  ...
  static isValidChoice(voteChoice, proposalChoices) {
    ...
    voteChoice.every((choice) => (proposalChoices === null || proposalChoices === void 0 ? void 0 : proposalChoices[choice - 1]) !== void 0) && // If any voteChoice is duplicated, return false
    voteChoice.length === new Set(voteChoice).size;
  }
  getValidVotes() {
    return this.votes.filter((vote2) => _ApprovalVoting.isValidChoice(vote2.choice, this.proposal.choices));
  }
  getScores() {
    return this.proposal.choices.map((choice, i) => this.getValidVotes().filter((vote2) => vote2.choice.includes(i + 1)).reduce((a, b) => a + b.balance, 0));
  }
  getScoresByStrategy() {
    return this.proposal.choices.map((choice, i) => this.strategies.map((strategy, sI) => this.getValidVotes().filter((vote2) => vote2.choice.includes(i + 1)).reduce((a, b) => a + b.scores[sI], 0)));
  }
  getScoresTotal() {
    return this.votes.reduce((a, b) => a + b.balance, 0);
  }
...
var QuadraticVoting = class _QuadraticVoting {
  ...
  static isValidChoice(voteChoice, proposalChoices) {
    ...
    Object.keys(voteChoice).every((key) => (proposalChoices === null || proposalChoices === void 0 ? void 0 : proposalChoices[Number(key) - 1]) !== void 0) && // If voteChoice object is empty, return false
    Object.keys(voteChoice).length > 0 && // If voteChoice object values have a negative number, return false
    Object.values(voteChoice).every((value) => typeof value === "number" && value >= 0) && // If voteChoice doesn't have any positive value, return false
    Object.values(voteChoice).some((value) => typeof value === "number" && value > 0);
  }
  getValidVotes() {
    return this.votes.filter((vote2) => _QuadraticVoting.isValidChoice(vote2.choice, this.proposal.choices));
  }
  getScores() {
    ...
    const quadraticScores = this.proposal.choices.map((_, i) => {
      const votingPowerSqrt = validVotes.map((vote2) => {
        const choiceWeightPercent = calcPercentageOfSum(vote2.choice[i + 1], Object.values(vote2.choice));
  ...
  getScoresByStrategy() {
    const validVotes = this.getValidVotes();
    const scoresTotal = this.getValidVotes().reduce((a, b) => a + b.balance, 0);
    const quadraticScoresByStrategy = this.proposal.choices.map((_, i) => this.strategies.map((_2, sI) => validVotes.map((vote2) => {
      const choiceWeightPercentByStrategy = calcPercentageOfSum(vote2.choice[i + 1], Object.values(vote2.choice));
...
function irv(ballots, rounds) {
  const candidates = [
    ...new Set(ballots.map((vote2) => vote2[0]).flat())
  ];
  ...
  const votesWithoutScore = votes.map((vote2) => [vote2[0], vote2[1][0]]);
...
function getFinalRound(votes) {
  const rounds = irv(votes.map((vote2) => [vote2.choice, vote2.balance, vote2.scores]), []);
  const finalRound = rounds[rounds.length - 1];
  return finalRound.sortedByHighest;
}
function getScoresMethod(votes, proposal2) {
  const finalRound = getFinalRound(votes);
  return proposal2.choices.map((choice, i) => finalRound.filter((res) => Number(res[0]) === i + 1).reduce((a, b) => a + b[1][0], 0));
}
var RankedChoiceVoting = class _RankedChoiceVoting {
  ...
  static isValidChoice(voteChoice, proposalChoices) {
    ...
    voteChoice.every((voteChoice2) => (proposalChoices === null || proposalChoices === void 0 ? void 0 : proposalChoices[voteChoice2 - 1]) !== void 0) && // If any voteChoice is duplicated, return false
  ...
  getValidVotes() {
    return this.votes.filter((vote2) => _RankedChoiceVoting.isValidChoice(vote2.choice, this.proposal.choices));
  }
  getScores() {
...
var WeightedVoting = class _WeightedVoting {
  ...
  getValidVotes() {
    return this.votes.filter((vote2) => _WeightedVoting.isValidChoice(vote2.choice, this.proposal.choices));
  }
  getScores() {
    const results = this.proposal.choices.map((choice, i) => this.getValidVotes().map((vote2) => weightedPower(i, vote2.choice, vote2.balance)).reduce((a, b) => a + b, 0));
  ...
  getScoresByStrategy() {
    const results = this.proposal.choices.map((choice, i) => this.strategies.map((strategy, sI) => this.getValidVotes().map((vote2) => weightedPower(i, vote2.choice, vote2.scores[sI])).reduce((a, b) => a + b, 0))).map((arr) => arr.map((pwr) => [pwr]));
    const validScoresTotal = this.getValidVotes().reduce((a, b) => a + b.balance, 0);
...
function getDelegatesBySpace(network_1, space_1) {
  return __awaiter(this, arguments, void 0, function* (network, space2, snapshot = "latest", options = {}) {
    ...
    const spaceIn = space2 ? buildSpaceIn(space2) : null;
    while (true) {
...
function buildSpaceIn(space2) {
  const spaces = ["", space2];
  if (space2.includes(".eth"))
    spaces.push(space2.replace(".eth", ""));
  return spaces;
}
...
var mainnetNetworkIds = Object.keys(networks).filter((id) => !networks[id].testnet);
ajv.addKeyword({
...
function call(provider, abi, call2, options) {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    try {
      ...
      return yield contract[call2[1]](...params, options || {});
    } catch (e) {
...
function subgraphRequest(url_1, query_1) {
  return __awaiter(this, arguments, void 0, function* (url, query, options = {}) {
    ...
    const res = yield (0, import_cross_fetch.default)(url, {
      method: "POST",
...
function getScores(space_1, strategies_1, network_1, addresses_1) {
  return __awaiter(this, arguments, void 0, function* (space2, strategies, network, addresses, snapshot = "latest", scoreApiUrl = DEFAULT_SCORE_API_URL, options = {}) {
...
function getVp(address, network, strategies, snapshot, space2, delegation, options) {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    try {
      const res = yield (0, import_cross_fetch.default)(url, init);
      const response = yield parseScoreAPIResponse(res);
...
function validate3(validation, author, space2, network, snapshot, params, options) {
  return __awaiter(this, void 0, void 0, function* () {
...
function getEnsTextRecord(ens_1, record_1) {
  return __awaiter(this, arguments, void 0, function* (ens, record, network = "1", options = {}) {
    var _a, _b;
    const { ensResolvers = ((_a = networks[network]) === null || _a === void 0 ? void 0 : _a.ensResolvers) || networks["1"].ensResolvers, broviderUrl } = options, multicallOptions = __rest(options, ["ensResolvers", "broviderUrl"]);
    let ensHash;
    try {
...
export {
  snapshot_esm_default as default
};
/*! Bundled license information:

@snapshot-labs/snapshot.js/dist/snapshot.esm.js:
  (*! *****************************************************************************
  ...
  ***************************************************************************** *)
*/
//# sourceMappingURL=@snapshot-labs_snapshot__js.js.map

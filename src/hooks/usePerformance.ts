...
export function usePerfromance() {
  const { fetchQuery } = useRestfulAPI();

  const loadingPerformanceHome = useRef(false);
  const loadingMorePerformanceHome = ref(false);
  ...
  function _fetchNewestPerformance(variables: any = {}, skip = 0) {
    return apolloQuery(
      {
